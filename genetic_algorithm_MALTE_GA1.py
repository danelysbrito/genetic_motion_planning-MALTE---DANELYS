...
def run_genetic_algorithm(env: Environment, population_size, max_generations, random_seed, visualize = True, individual_solution = True):
    solver = GeneticSolver(len(env.maklink.path)-2, population_size, max_generations, env, individual_solution, random_seed)
    ...
    if visualize:
        env.visualize_environment(solution, True, algorithm="GA_1")
